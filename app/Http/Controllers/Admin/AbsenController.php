...
class AbsenController extends Controller
{
    public function index($id,$id_jadwal) {
    	...
    	$absen = DB::table('absen')
    				...
    				->get();
    	return view('Admin.absen.main',compact('absen','title','page','id','jadwal','id_jadwal'));
    }

    public function form($id,$id_jadwal) {
    	$title = 'Form Absen';
    	$page = 'kegiatan';
    	return view('Admin.absen.form-absen',compact('title','page','id','id_jadwal'));
    }

    public function delete($id,$id_jadwal,$id_absen) {
    	Absen::where('id_jadwal',$id_jadwal)->where('id_absen',$id_absen)->delete();
    	return redirect('/admin/kegiatan/'.$id.'/jadwal/'.$id_jadwal.'/absen')->with('message','Berhasil Hapus Absen');
    }
    ...
    public function save(Request $request) {
        ...
        if ($kegiatanDetail->count() == 0) {
            return redirect('/admin/kegiatan/'.$id_kegiatan.'/jadwal/'.$id_jadwal.'/absen')->with('log','Maaf Peserta Belum Terdaftar Pada Kegiatan');
        }
        else {
            $get = $kegiatanDetail->firstOrFail();
            $count = Absen::where('id_detail',$get->id_detail)->where('id_jadwal',$id_jadwal)->count();
            if ($count != 0) {
                return redirect('/admin/kegiatan/'.$id_kegiatan.'/jadwal/'.$id_jadwal.'/absen')->with('log','Maaf Peserta Sudah Ter input');
            }
            else {
                $array = [
                ...
                Absen::create($array);
                return redirect('/admin/kegiatan/'.$id_kegiatan.'/jadwal/'.$id_jadwal.'/absen')->with('message','Berhasil Input Absen');
            }
        }
    }
}
